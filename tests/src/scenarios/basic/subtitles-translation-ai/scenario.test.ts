...
import { subtitlesTranslationAiUser } from './scenario'
...
it(
  'Subtitles translation (AI user, .srt -> EN->RU)',
  { timeout: TIMEOUT },
  async () => {
    if (skip()) return
    const result = await runAiScenario(subtitlesTranslationAiUser)
...
it(
  'Subtitles translation (AI user) — trace',
  { timeout: TIMEOUT },
  async () => {
    if (skip()) return
    const result = await runAiScenario(subtitlesTranslationAiUser)
